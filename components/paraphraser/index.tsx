...
export default function Paraphraser(props) {
  const [paraphraseMode, setParaphraseMode] = useState("AES");
  const [encryptDecryptMode, setEncryptecryptMode] = useState("encrypt");
  ...
  const handleParaphraseSubmission = () => {
    (async () => {
      ...
      const loadingToast = toast.loading(`{encryptDecryptMode}ing...`);
      ...
      }).catch((_err) => { toast.error('We ran into an issue when trying to encrypt. Please try again later.'); });
            ...
            <div className="p-4 sm:px-0">
              <div className="grid grid-cols-2 gap-x-1">
                ...
                <label htmlFor="inputText">
                  ...
                  <textarea name="inputText" className="block w-full p-4 border-2 border-gray-200 rounded-lg resize-none h-24 disabled:opacity-60 sm:text-sm md:text-lg focus:outline-none focus:ring focus:border-blue-600" placeholder="Enter the text you want to paraphrase. You can select any of the modes above for different levels of paraphrasing. After writing or pasting your text, use the Paraphrase button below." value={inputText} onChange={handleInputTextChange} disabled={paraphraseLoading} ref={inputTextRef}></textarea>
                </label>
              </div>
            </div>
